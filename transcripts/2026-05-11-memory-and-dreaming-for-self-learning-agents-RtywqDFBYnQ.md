---
description: Transcription brute de la vidéo YouTube "Memory and dreaming for self-learning agents".
...
[0:07] Hello.
[0:08] >> Hey everyone. How's it going? Thanks for
[0:11] coming.
[0:14] My name is Mahes and I'm a product
[0:16] manager on the platform team here at
[0:19] Anthropic. Uh over the past year and a
[0:21] half, I've gotten to work on primitives
[0:24] like MCP and Skills. And today I want to
[0:27] talk about the primitive that I'm most
[0:28] excited about next, which is memory. Um,
[0:32] I'll talk about why we think memory is
[0:34] so important and why we've been spending
[0:36] so much time on it at Enthropic. How we
[0:38] think about designing memory systems
[0:40] that are built for Frontier Agents. And
[0:43] I'm excited to also talk about Dreaming,
[0:45] a brand new product that we're launching
[0:47] today in research preview in the managed
[0:50] agents API.
[0:55] Model capabilities have improved really
[0:58] quickly over the last couple of years
[1:00] and agents are capable of tasks that
[1:03] take many many hours and can run for
[1:06] hours or almost days at a time. And as
[1:08] models and agents have improved, we've
[1:11] also invested in building higher and
[1:13] higher level capabilities and primitives
[1:15] that kind of get out of those models way
[1:18] and give them access to additional bits
[1:21] of their environment and things that
[1:22] they can manage and become more powerful
[1:25] over time. So for example, we launched
[1:28] MCP which gives agents access to
[1:31] external tools and data. We launched
[1:34] harnesses that were really powerful like
[1:36] claude code and the agent SDK. And in
[1:39] October, we launched skills which let
[1:41] agents pick up brand new capabilities
[1:43] that either other agents have designed
[1:45] and shared with them or humans and users
[1:48] that they interact with have designed
[1:49] for them. Each primitive has let agents
[1:53] do increasingly powerful things for
[1:55] longer periods of time. But we still
[1:58] think that something is still kind of
[2:00] unsolved and that's continuous
[2:02] self-learning and context management
[2:04] over long horizon tasks.
[2:08] So memory is the next primitive. It's
[2:11] the thing that I think will get us to
[2:13] self-learning agents that evolve and
[2:15] improve based on the tasks that they're
[2:18] working on and their own experience.
[2:20] With memory, agents can learn about the
[2:23] tasks they work on. things like the
[2:25] success criteria, common mistakes,
[2:27] strategies that are or are not working.
[2:30] They can learn about their environments,
[2:32] things like the code bases that they
[2:34] interact with, the files and the assets
[2:36] that they're constantly keeping up to
[2:38] date. And they can also learn from other
[2:40] agents that are in the same environment
[2:42] as them. They can share learnings. They
[2:44] can figure out what's going wrong
[2:46] elsewhere in a system and incorporate
[2:48] that into their own memory. And I think
[2:50] this last point is the one that I've
[2:52] been most excited about this year and
[2:54] over the next couple of months. I think
[2:56] self-managed memory is going to be super
[2:58] important in these large and complex
[3:00] multi-agent systems where a swarm of
[3:02] agents that are working in a similar
[3:04] environment on discrete tasks are
[3:07] essentially building up their own
[3:09] understanding, their own model of the
[3:11] world that they're in over time.
[3:15] So to help get to this vision, we just
[3:17] launched memory in cloud managed agents
[3:19] in public beta a couple of weeks ago.
[3:22] This gives developers a frontier memory
[3:24] system that works out of the box to
[3:27] maximize intelligence by default to
[3:30] support these systems of many agents
[3:32] running concurrently in the same
[3:34] environment. and most importantly to
[3:36] give enterprises and developers the
[3:39] flexibility and control they need to
[3:41] actually run these in production in an
[3:43] enterprise setting.
[3:46] We've already heard from a bunch of
[3:48] teams building on this to date um that
[3:50] all say that this helps them get to
[3:52] continuous learning and continuous
[3:54] improving agents a lot faster. Uh
[3:57] Rockutin for example mentioned that uh
[3:59] this helped them drop their first past
[4:01] mistakes in their internal uh knowledge
[4:03] agents by 90% because agents were able
[4:06] to catch mistakes and share them with
[4:08] the next iteration of agents which also
[4:11] led to better token efficiency and lower
[4:13] costs and better latency because they
[4:15] started deploying memory systems.
[4:20] So I want to talk a bit about the
[4:22] requirements that we kept in mind uh
[4:24] that we discovered while talking to
[4:26] customers and building this. The first
[4:28] and most important is memory needs to be
[4:31] built to maximize intelligence by
[4:33] default. Agent builders have uh been
[4:36] designing memory systems for a while. I
[4:37] mean we ourselves launched cloud.md uh
[4:40] originally with cloud code I think uh
[4:42] about a year and a half ago. And this
[4:44] was a pretty constrained early version
[4:46] of memory where an agent could leave
[4:48] notes for itself. Sometimes the user
[4:50] would also leave notes in the same
[4:52] memory file. And we also launched
[4:54] something like the memory tool uh within
[4:56] our SDKs which was a pretty well
[4:59] specified tool call with specific
[5:01] parameters and output formats that uh
[5:04] API builders could use. As agents have
[5:07] improved, we've tried to get more and
[5:09] more out of Claude's way and delegate
[5:11] more of this decision-making to Claude
[5:13] without over constraining um the design
[5:16] of these harnesses. And as we did with
[5:19] skills, we kind of came to the
[5:20] conclusion that hey, we know that agents
[5:23] are able to manage a virtual environment
[5:25] and manage their own file system. So why
[5:27] can't we go the same direction with
[5:29] memory? Memory in cloud managed agents
[5:32] models memory as a file system to
[5:35] claude. a series of files with a
[5:37] specific hierarchy and format that
[5:39] Claude can manage and update on its own.
[5:42] It can use familiar tools like bash and
[5:45] GP to update this memory to keep it
[5:47] organized and to constantly change it as
[5:50] it starts working on a task. Now, this
[5:53] this also tracks with what we're seeing
[5:55] in the latest models with Claude Opus
[5:57] 4.7, which we just launched last month.
[6:00] We saw that it was state-of-the-art at
[6:01] file system based memory. That means
[6:04] it's a lot better at discerning what
[6:06] content to put into memory, what's worth
[6:08] remembering. It's better at figuring out
[6:11] what's the right structure for memory.
[6:12] How many files should I split memory
[6:14] into? How do I keep it organized inside
[6:17] of a file system? And ultimately, it all
[6:19] does this with just bash tools and GP
[6:22] tools that already make Claude so good
[6:24] at agentic coding.
[6:29] The other thing that we had in mind when
[6:31] designing memory is that it needs to
[6:33] scale with the multi- aent systems that
[6:35] we're going to be building over the
[6:36] coming months. Multi multi parallel
[6:39] agents is something that we're already
[6:40] kind of starting to do with cloud code.
[6:42] There's a lot of uh vibe coders that
[6:44] have like 10 or 15 cloud code sessions
[6:47] running at the same time. And we're
[6:48] starting to see this in an enterprise
[6:50] setting as well where enterprises
[6:52] including Antropic have hundreds or
[6:54] sometimes even thousands of agents
[6:56] running in parallel interacting with the
[6:58] same set of shared state and the same
[7:00] shared memory. So there's a couple of
[7:02] properties that come out of this. One is
[7:05] we want to give agents the ability to
[7:07] mix and match between the session and
[7:09] the work that it's doing and the set of
[7:12] memory stores that it has access to. So
[7:15] one property of memory and managed
[7:17] agents is permission scopes. The ability
[7:19] for one agent to have readonly access to
[7:22] one memory store. And maybe that memory
[7:24] store is organizationwide knowledge, uh
[7:27] a set of best practices, a runbook for
[7:30] how to deal with these common tasks. And
[7:32] then it has readwrite memory for another
[7:34] memory store. So maybe that's another
[7:36] one where it has working memory that's a
[7:39] lot more specific and frequently updated
[7:41] based on the work it's doing.
[7:44] The other property that came out of this
[7:46] was concurrency. If there are hundreds
[7:48] or thousands of agents interacting at
[7:50] the same time with the same uh memory
[7:53] state, it needs to be able to know that
[7:55] it's not going to clobber the memory or
[7:57] overwrite it as it continues working.
[7:59] So, we implemented optimistic
[8:00] concurrency where one agent can
[8:03] essentially use a content hash to um
[8:05] check if it's going to overwrite another
[8:07] agent's memory before it actually makes
[8:09] an update.
[8:11] From
[8:13] talking to customers, the final and most
[8:16] important property from all of this is
[8:18] about developer and enterprise control
[8:20] for actual production agents. A couple
[8:23] of things came out of this. The first
[8:25] and probably most sought-after property
[8:27] is version history. It's the ability for
[8:31] the developers building with managed
[8:32] agents to see an entire audit log of
[8:36] every time memory was updated and to
[8:38] actually even give an agents access to
[8:41] the same audit log in the future so they
[8:43] can keep track of what change was made
[8:44] and when. It's also attribution metadata
[8:48] to say what agent made an update, what
[8:51] time did it make that update, what
[8:52] session made that specific change, and
[8:54] to go super granularly. So this is
[8:56] predictable and in developers control.
[9:00] The other property that came out of this
[9:02] was a standalone API. We talked to a lot
[9:05] of customers that are building bespoke
[9:07] systems outside of manage agents to
[9:09] manage and curate their memory and keep
[9:11] it up to date. We talked to customers
[9:13] that do PII scanning to make sure that
[9:15] memory doesn't have sensitive content
[9:17] that shouldn't be in there. We also
[9:19] talked to customers that wanted to clean
[9:21] up memory in their own separate pipeline
[9:23] or to clone it into external systems. So
[9:26] we didn't want to lock them in into a
[9:28] specific system that was only available
[9:29] to manage agents. Uh we built this
[9:31] portable API so they could go and
[9:33] control these additional things on their
[9:34] own.
[9:39] So taking a step back, we've started to
[9:41] form this picture of the different
[9:43] layers that we need to work in as we
[9:45] build a frontier memory system. We've
[9:47] talked about the storage layer, which is
[9:50] where the data is actually stored, what
[9:52] kind of metadata and attribution data
[9:55] we're leaving alongside of it. We've
[9:57] talked about the structure and the
[9:59] content layer. This is things like our
...
[16:38] super efficiently and performatively. So
